...
class Account:
    ...
    def update_func(self):
        if self.fname_var.get() == "" or self.lname_var.get() == "" or self.username_var.get() == "" or self.email_var.get() == "" or self.password_var.get() == "" or self.confirm_password_var.get() == "" or self.contact_var.get() == "":
            ...
            try:
                ...
                if open_main == 1:
                    query4 = "update admin_details set fname = %s, lname = %s, contact = %s, email = %s, username = %s where (email = %s and username = %s)"
                    values4 = ( 
    ...
    def change_password_func(self):
        if self.old_password_var.get() == "" or self.reset_password_var.get() == "" or self.reset_confirm_password_var.get() == "":
            ...
            try:
                connection = mysql.connector.connect(host = "localhost", username = "root", password = "Root@123", database = "travel_ms_db")
                my_cursor = connection.cursor()
                query3 = "select * from admin_details where (email = %s and username = %s) and new_password = %s"
                ...
                if row == None:
                    ...
                    query4 = "update admin_details set new_password = %s where (email = %s and username = %s)"
                    values4 = ( 
        ...
        if self.fname_var.get() == "" or self.lname_var.get() == "" or self.username_var.get() == "" or self.email_var.get() == "" or self.password_var.get() == "" or self.confirm_password_var.get() == "" or self.contact_var.get() == "":
            ...
            try:
                ...
                if open_main == 1:
                    ...
                    query3 = "delete from admin_details where (email = %s and username = %s)"
    ...
    def fetch_account_data(self):
        try:
            connection = mysql.connector.connect(host = "localhost", username = "root", password = "Root@123", database = "travel_ms_db")
            my_cursor = connection.cursor()
            query3 = "select * from admin_details where (email = %s and username = %s)"
            values3 = (self.email_var.get(), self.username_var.get())
            my_cursor.execute(query3, values3)
            row = my_cursor.fetchone()
            if row != None:
                from admin_dashboard import Travel
